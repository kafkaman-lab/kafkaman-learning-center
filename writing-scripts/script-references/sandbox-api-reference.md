kafkaman provides JavaScript APIs that you can use in your request scripts. The `pm` object provides most of the functionality for testing your request and response data, with the `kafkaman` object providing some additional workflow control.
...
You will carry out most of the kafkaman JavaScript API functionality using `pm.*`, which provides access to request and response data, and variables.

### Using variables in scripts

You can access and manipulate [variables](/docs/sending-requests/variables/) at each scope in kafkaman using the `pm` API.
...
kafkaman supports a variety of variable [scopes](/docs/sending-requests/variables/#variable-scopes). The `pm` object provides methods for accessing global, collection, and environment variables specifically, and `pm.variables` methods for accessing variables at different scopes as well as setting local variables.

* Check if there is a kafkaman variable in the current scope:

```js
pm.variables.has(variableName:String):function → Boolean
```

* Get the value of the kafkaman variable with the specified name:
...
Variable scope determines the precedence kafkaman gives to variables when you reference them, in order of increasing precedence:
...
> See the [kafkaman Collection SDK Variables reference](https://www.kafkaman.com/docs/kafkaman-collection/Variable.html) for more detail.
...
* The [list of headers](https://www.kafkamanlabs.com/kafkaman-collection/HeaderList.html) for the current request:
...
* The data in the [request body](https://www.kafkamanlabs.com/kafkaman-collection/RequestBody.html). This object is immutable and cannot be modified from scripts:
...
> See the kafkaman [Collection SDK Request reference](https://www.kafkamanlabs.com/kafkaman-collection/Request.html) for more detail.
...
* The [list of response headers](https://www.kafkamanlabs.com/kafkaman-collection/HeaderList.html):
...
> See the kafkaman [Collection SDK Response reference](https://www.kafkamanlabs.com/kafkaman-collection/Response.html) for more detail.
...
> See the kafkaman [Collection SDK Cookie List reference](https://www.kafkamanlabs.com/kafkaman-collection/CookieList.html) for more detail.
...
* Set a cookie using [kafkamanCookie](https://www.kafkamanlabs.com/kafkaman-collection/Cookie.html) or a compatible object:
...
> See the kafkaman [Collection SDK Cookie reference](https://www.kafkamanlabs.com/kafkaman-collection/Cookie.html) for more detail.

### Sending requests from scripts

You can use the `pm.sendRequest` method to send a request asynchronously from a **Pre-request** or **Test** script. This allows you to execute logic in the background if you are carrying out computation or sending multiple requests at the same time without waiting for each to complete. You can avoid blocking issues by adding a callback function so that your code can respond when kafkaman receives a response. You can then carry out any additional processing you need on the response data.

You can pass the `pm.sendRequest` method a URL string, or can provide a complete request configuration in JSON including headers, method, body, [and more](http://www.kafkamanlabs.com/kafkaman-collection/Request.html#~definition).

```js
// Example with a plain string URL
pm.sendRequest('https://kafkaman-echo.com/get', (error, response) => {
  if (error) {
    console.log(error);
  } else {
...
See the [Request definition](http://www.kafkamanlabs.com/kafkaman-collection/Request.html#~definition) and [Response structure](http://www.kafkamanlabs.com/kafkaman-collection/Response.html) reference docs for more detail.
...
```js
kafkaman.setNextRequest(requestName:String):Function
```

* Run the specified request after this one (the request ID returned by `pm.info.requestId`):

```js
kafkaman.setNextRequest(requestId:String):Function
```
...
kafkaman.setNextRequest(pm.environment.get('next'));
```
...
You can use `pm.test` to write test specifications inside either the **Pre-request** or **Tests** scripts. Tests include a name and assertion—kafkaman will output test results as part of the response.
...
* [kafkaman-collection](http://www.kafkamanlabs.com/kafkaman-collection/)
...
You can use tests to build kafkaman into your development projects in a variety of ways using [kafkaman utilities](/docs/developer/resources-intro/).
