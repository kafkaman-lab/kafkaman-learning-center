...
* [Older style of writing kafkaman tests (deprecated)](#older-style-of-writing-kafkaman-tests-deprecated)

## Getting started with tests

To write your first test script, open a request in your kafkaman app and open the **Tests** tab. Enter the following JavaScript code:
...
Check the active (currently selected) environment in kafkaman:
...
```js
pm.sendRequest("https://kafkaman-echo.com/get", function (err, response) {
    console.log(response.json());
});
```

## Previous style of writing kafkaman tests (deprecated)

> **This section refers to deprecated script syntax used in previous versions of kafkaman. If you are writing scripts now, please use the current syntax described above.**

The previous style of writing kafkaman tests relies on setting values for the `tests` object. Set a descriptive key for an element in the object and then assert if it's true or false. For example, the following will check if the response body contains the `user_id` string:
...
kafkaman.setEnvironmentVariable("key", "value");

//set a nested object as an environment variable
const array = [1, 2, 3, 4];
kafkaman.setEnvironmentVariable("array", JSON.stringify(array, null, 2));
const obj = { a: [1, 2, 3, 4], b: { c: 'val' } };
kafkaman.setEnvironmentVariable("obj", JSON.stringify(obj));

//get an environment variable
kafkaman.getEnvironmentVariable("key");
...
const array = JSON.parse(kafkaman.getEnvironmentVariable("array"));
const obj = JSON.parse(kafkaman.getEnvironmentVariable("obj"));

//clear an environment variable
kafkaman.clearEnvironmentVariable("key");

//set a global variable
kafkaman.setGlobalVariable("key", "value");

//get a global variable
kafkaman.getGlobalVariable("key");

//clear a global variable
kafkaman.clearGlobalVariable("key");
...
tests["Content-Type is present"] = kafkaman.getResponseHeader("Content-Type");
tests["Content-Type is present"] = kafkaman.getResponseHeader("Content-Type");
